...
AV_DELTA_DEC = {"5-8 and 17-20": 0.065, "0-5, 8-17, and 20-0": 0.182}
...
def calculate_moon_ra_dec(fix_ra: List[int], fix_dec: List[int], start_date, av_delta_ra: Dict[str, float],
    ...
    for sec in range(1, time_in_seconds):
        if 0 <= new_ra < 5 * 3600:
            ...
            new_dec = new_dec + av_delta_dec.get("0-5, 8-17, and 20-0")
        elif 5 * 3600 <= new_ra < 7 * 3600:
            new_ra = new_ra + av_delta_ra.get("5-12")
            new_dec = new_dec + av_delta_dec.get("5-8 and 17-20")
        elif 7 * 3600 <= new_ra < 8 * 3600:
            new_ra = new_ra + av_delta_ra.get("5-12")
            new_dec = new_dec - av_delta_dec.get("5-8 and 17-20")
